...
public class InputHandler {
    ...
    private boolean aabbIntersects(
    ...
    private boolean intersectsPlayer(int bx, int by, int bz) {
        ...
        return aabbIntersects(pMinX, pMinY, pMinZ, pMaxX, pMaxY, pMaxZ,
    ...
    public static final class Hit {
        ...
        public final int nx, ny, nz; // outward normal of the face hit (pointing toward the camera)
    ...
    public Hit pickBlockFromCamera() {
        ...
        // Forward dir in your coordinate system
        ...
        if (stepX > 0) tMaxX = ((vx + 1) - eye.x) * invDx; else if (stepX < 0) tMaxX = (eye.x - vx) * invDx; else tMaxX = Float.POSITIVE_INFINITY;
        if (stepY > 0) tMaxY = ((vy + 1) - eye.y) * invDy; else if (stepY < 0) tMaxY = (eye.y - vy) * invDy; else tMaxY = Float.POSITIVE_INFINITY;
        if (stepZ > 0) tMaxZ = ((vz + 1) - eye.z) * invDz; else if (stepZ < 0) tMaxZ = (eye.z - vz) * invDz; else tMaxZ = Float.POSITIVE_INFINITY;
    ...
    public void pollEvents(float delta) {
        // STEP 1: vertical physics (gravity, swim, ground snap) — handled in PhysicsEngine
        physics.update(delta, isJumpPressed(), isCrouchPressed());

        // STEP 2: compute intended horizontal motion
        ...
        if (sprint) moveSpeed = 0.15f;     // faster when sprinting
        if (crouch) moveSpeed = 0.06f;     // slower when crouched
        ...
        // STEP 3: apply horizontal movement with collision queries from PhysicsEngine
