...
public class AnimatedTexture extends Texture {
    ...
    public void bind() {
        ...
        if (id != 0) GL30.glBindTexture(GL30.GL_TEXTURE_2D, id);

        int currentProgram = GL11.glGetInteger(GL20.GL_CURRENT_PROGRAM);
        if (currentProgram != 0) {
            int totalHeight = this.height;
            int framePixels = this.width;
            if (totalHeight <= 0 || framePixels <= 0) return;
            ...
            if (offLoc >= 0)   GL20.glUniform1f(offLoc, offset);
            if (scaleLoc >= 0) GL20.glUniform1f(scaleLoc, scale);
        }
    }

    public int getFrames() { return frames; }
    public int getCurrentFrame() { return currentFrame; }
    public float getFrameHeightFraction() { return frameHeightFraction; }
}